 ...
 * 
 * @author scblack, modified by rcardone
 * 
 */
@Test(groups= {"unit"})
public class ASTParserTest
{
   ...
   */
  @Test(groups = {"unit"})
  public void testASTParseValid() throws InvalidSelectorException
  {
    ...
    for (Map.Entry<Integer, CaseData> item : validCaseInputs.entrySet())
    {
      ...
      var ast = SqlExprParser.parse(ci.sqlStr);
   ...
   * The leaf count was used in the original Tapis code that defined its own AST nodes, 
   * but doesn't work well for the apache nodes returned by SqlExprParser.  The 
   * leafCount field is currently ignored.
   */
  static class CaseData
  {
    public final int leafCount; // Number of expected leaf nodes in the AST
    public final String sqlStr;
    CaseData(int r, String s) { leafCount=r; sqlStr =s; }
  }
}
